...
def is_ocsr(serial_schedule, must_comes_before):
    ...
    return True, f"<br>Serial Schedule Conflict Equivalent and OCSR: <strong>{''.join(serial_schedule_to_print)}</strong>"
...
def solveOCSR(schedule):
    ...
    if possible_serial_schedules != None:
        for serial_schedule in possible_serial_schedules:
            return is_ocsr(serial_schedule, must_comes_before)
